...
vim.keymap.set('n', 'h', '<cmd>echo "Use j instead!!"<CR>')
...
-- motion keys ==> (left, down, up, right) = (j,k,l,æ)
vim.keymap.set({ 'n', 'v' }, 'j', 'h')
vim.keymap.set({ 'n', 'v' }, 'k', 'j')
vim.keymap.set({ 'n', 'v' }, 'l', 'k')
vim.keymap.set({ 'n', 'v' }, 'æ', 'l')

-- prufa að setja stóra H til að join-line  

-- Nota alt til að færa á milli ramma fyrir terminal og insert
vim.keymap.set({ "t", "i" }, "<A-j>", "<C-\\><C-N><C-w>h")
vim.keymap.set({ "t", "i" }, "<A-k>", "<C-\\><C-N><C-w>j")
vim.keymap.set({ "t", "i" }, "<A-l>", "<C-\\><C-N><C-w>k")
vim.keymap.set({ "t", "i" }, "<A-æ>", "<C-\\><C-N><C-w>l")
...
vim.keymap.set({ 'n', 'v' }, 'J', '^', { desc = 'Start of the line' })
vim.keymap.set({ 'n', 'v' }, 'Æ', '$', { desc = 'End of the line' })

-- ef Oil er ekki tilstaðar
vim.keymap.set("n", "<leader>e", "<cmd>Ex<cr>", { desc = 'Open netrw Explore' })

-- Hraðari færsla
vim.keymap.set('n', 'K', '10j', { desc = '10 lines down' })
vim.keymap.set('n', 'L', '10k', { desc = '10 lines up' })
